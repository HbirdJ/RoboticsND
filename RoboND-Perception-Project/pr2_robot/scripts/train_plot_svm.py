...
for x in xrange(0,8):
    ...
    tics = [16,48,80,112,144,176]
    labels = ['h','s','v','xnorm', 'ynorm', 'znorm']
    plt.xticks(tics, labels)
    
    for y in xrange(x*25, (x+1)*25):
        plt.plot(feature_list[y])
    plt.title('Vectors for %s' %label_list[x*25], fontsize=30)
    plt.tick_params(axis='both', which='major', labelsize=20)
    fig.tight_layout()
    filename = 'plots/%s.png' %label_list[x*25] 
...
# Create classifier
clf = svm.SVC(gamma='auto', C=1e3, kernel='linear')
